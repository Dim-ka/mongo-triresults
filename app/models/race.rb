class Race
  ...
  def get_group racer
    if racer && racer.birth_year && racer.gender
      ...
      gender=racer.gender
      name=min_age >= 60 ? "masters #{gender}" :
  ...
  def create_entrant racer
    ...
    entrant.racer=racer.info.attributes.symbolize_keys
  ...
  def self.upcoming_available_to racer
    registered_races_ids = racer.races.upcoming.pluck(:race).map {|r| r[:_id]}
    Race.upcoming.where(:_id.nin=>registered_races_ids)
  end
end
