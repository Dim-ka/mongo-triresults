class Entrant
  ...
  # FIXIT! must be modified
  def update_total(result)
    self.secs = results.inject(0) { |sum, val| sum||=0; sum + val.secs if val.secs }
  end
  ...
  RESULTS.keys.each do |name|
    #create_or_find_result
    define_method("#{name}") do
      result=results.select{|result| name==result.event.name if result.event }.
        first
      if !result
